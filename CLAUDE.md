...
5. **Lector KYN obligatorio**: el bloque de accesibilidad de lectura (lectura biónica con 3 niveles, regla que sigue el cursor, tamaño A−/A+, localStorage, oculto en print, y modo mini: la barra se encoge a un botoncito ⚡ a los segundos de no usarla). **Ya NO se copia**: es un **bundle compartido** en `proyectos/KYN Design System/lector-kyn/` (fuente única). Enlazarlo con ruta relativa: `<link rel="stylesheet" href="../KYN%20Design%20System/lector-kyn/lector-kyn.css">` en el `<head>` y `<script defer src="../KYN%20Design%20System/lector-kyn/lector-kyn.js"></script>` antes de `</body>`. La barra y la regla se auto-inyectan; el selector de texto a bionizar se puede sobreescribir con `window.LECTOR_KYN_SELECTOR = '...'` antes del script. (El componente Vue `LectorKYN.vue` de Motion Lab es aparte: trae además la mirada/iris; no usa este bundle.)
