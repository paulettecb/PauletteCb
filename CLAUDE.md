...
5. **Lector KYN obligatorio (package compartido, NO copiar)**: el lector vive como módulo único en `proyectos/packages/lector-kyn/`. **Enlázalo, no lo copies** — 2 líneas antes del cierre del body:
   ...
   ```
   Trae: biónica (3 niveles), regla que sigue el cursor con grosor ajustable, tamaño A−/A+, localStorage (`lector-kyn`), modo mini ⚡, oculto en print, y **👁️ Mirada** (regla que sigue los ojos por webcam/iris, opt-in, todo local, con calibración 2D máxima o rápida). La mate de la mirada es fuente única en `packages/lector-kyn/gaze/gaze-math.js` (la comparte el componente Vue de Motion Lab). **Ojo**: al ser módulo ESM + WASM, el lector solo corre en el sitio publicado o con servidor local (`npm run dev`), no abriendo el HTML como `file://`.
