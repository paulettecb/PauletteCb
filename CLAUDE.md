...
2. **Estructura fija**: portada (sello + h1 Friendship + subtítulo Farmhouse + chips pastel), índice clickeable, capítulos con eyebrow Farmhouse, y cierre con glosario/chuleta. **Lectura por capítulo**: solo se muestra el capítulo activo; anterior/siguiente aparecen solo vía los botones `.cap-nav` (al imprimir se muestran todos). Títulos internos (h3): Hanken Grotesk (Google Fonts, la sans oficial de KYN) SIEMPRE en mayúsculas y weight 800 — a Paulette solo le gustan así.
